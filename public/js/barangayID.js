document.getElementById("barangay-ID").addEventListener("click", e => {
  console.log("qqqqq");
  ...
  <div
    ...
    <div class="flex justify-between">
      ...
      <h2 class="text-4xl font-oswald">Barangay ID Application</h2>
  ...
  <div
    ...
    <table class="max-w-full min-w-full">
      ...
      <tbody class="bg-white"  id="barangay-id-table">
  `;

  var barangayDoc = localStorage.getItem('barangay');
  db.collection('barangays').doc(barangayDoc).collection('requests').where("type", "in", ["barangay_id", "barangay_id_replacement"]).get()
    .then((querySnapshot) => {
      ...
      <div class="w-full max-w-3xl p-5 relative mx-auto my-auto rounded-xl shadow-lg  bg-white pb-4">
        ...
        <div class="">
          ...
          <i
              class="fa fa-copy text-3xl mr-2" aria-hidden=" true"></i>
            <h2 class="text-xl font-bold py-2 ">Barangay ID</h3>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                  <div class="px-6 bg-white border-b border-gray-200 h-96 overflow-scroll">
                    <form>
                      ...
                      <div class="mb-4">
                        <label class="text-xl text-gray-600">Address</label></br>
                      ...
                      <div class="mb-4">
                        ...
                        <input type="text" class="border-2 border-gray-300 p-2 w-full" name="type" value="" id="typeValue"
                          disabled>
                      </div>
                  ...
                  <div class="flex flex-col w-full">
                    <button role="submit"
                      ...
                      id="submitRequestBarangayID">Submit</button>
                    </div>
                    <div class="flex flex-col  w-full">
                      <button class="p-3 bg-red-500 text-white hover:bg-red-600 rounded-md"
                        id="cancelRequestBarangayID">Cancel</button>
                    </div>
                </div>
              </div>
            </div>
            ...
            <div class="py-4 w-3/5 mx-auto sm:px-6 lg:px-8 max-h-3/4 overflow-scroll">
              <div class="mb-4">
                <div class=" w-full flex flex-row justify-between">
                  <label class="text-xl text-gray-600">1x1 Picture</label>
                </div>

                <img src="https://via.placeholder.com/400x200" id="1x1Image">
              </div>

              <div class="mb-4">
                <div class=" w-full flex flex-row justify-between">
                  <label class="text-xl text-gray-600">Cedula</label>

                </div>
                
                <img src="https://via.placeholder.com/400x200" id="cedulaProofImage">
      ...
      document.getElementById("barangay-id-table").addEventListener("click", e => {
        e.preventDefault();
        if (e.target.id == "view-request-details") {
          document.getElementById("verify-id-modal").classList.remove("hidden");
          ...
          db.collection('barangays').doc(barangayDoc).collection('requests').doc(requestID).get()
            .then((doc) => {
              ...
              if (doc.data()["type"] == "barangay_id") {
                document.getElementById("typeValue").value = "New";
                reqType = "New";
              } else {
                document.getElementById("typeValue").value = "Replacement";
                reqType = "Replacement";
              }
              ...
              storageRef.child(`REQUEST/${doc.data()["storage_id"]}/1x1`).getDownloadURL().then(function (url) {
                console.log(url)
                document.getElementById('1x1Image').setAttribute('src', url);
              }).catch(function (error) {
                console.log(error.message)
              });
      ...
      document.getElementById("cancelRequestBarangayID").addEventListener("click", e => {
        e.preventDefault();
        document.getElementById("verify-id-modal").classList.add("hidden");
        console.log("cancel");
      });

      document.getElementById("submitRequestBarangayID").addEventListener("click", e => {
        e.preventDefault();
        ...
        db.collection('barangays').doc(barangayDoc).collection('requests').doc(requestID).update({
          ...
          if (statusValue == "Completed" && reqType == "New") {
            db.collection('barangays').doc(barangayDoc).collection('users').doc(authorID).update({
              barangay_id: true,
              barangay_id_replacement: false,
            }).then(function () {
              console.log("New Barangay ID set to True:");
            })
          } else if (statusValue != "Completed" && reqType == "New") {
            db.collection('barangays').doc(barangayDoc).collection('users').doc(authorID).update({
              barangay_id: false,
              barangay_id_replacement: false,
            }).then(function () {
              console.log("Replacement Barangay ID set to False:");
            })
          } else if (statusValue == "Completed" && reqType == "Replacement") {
            db.collection('barangays').doc(barangayDoc).collection('users').doc(authorID).update({
              barangay_id_replacement: false,
            }).then(function () {
              console.log("Replacement Barangay ID set to False:");
            ...
            db.collection('barangays').doc(barangayDoc).collection('users').doc(authorID).update({
              barangay_id_replacement: true,
            }).then(function () {
              console.log("Replacement Barangay ID set to True:");
            })
          }
        ...
        document.getElementById('barangay-ID').click();
...
document.getElementById("health-certificate").addEventListener("click", e => {
  e.preventDefault();

  content.innerHTML =
    `
  <div class="flex flex-col items-center justify-center h-full">
    <img src="../img/white_logo.png" alt="white logo" class="w-80 mb-8">
    <h1 class="text-gray-500">Health Certificate Page is Under Construction</h1>
  </div>
  `
});
