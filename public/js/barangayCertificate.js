document.getElementById("certificate-barangay").addEventListener("click", e => {
  console.log("qqqqq");
  ...
  <div
    ...
    <table class="max-w-full min-w-full">
      ...
      <tbody class="bg-white"  id="barangay-cert-table">
  `;

  var barangayDoc = localStorage.getItem('barangay');
  db.collection('barangays').doc(barangayDoc).collection('requests').where("type", "==", "barangay_certificate").get()
    .then((querySnapshot) => {
      ...
      <div class="w-full max-w-3xl p-5 relative mx-auto my-auto rounded-xl shadow-lg  bg-white pb-4">
        ...
        <div class="">
          ...
          <i
            ...
            <h2 class="text-xl font-bold py-2 ">Barangay Certificate</h3>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                  <div class="px-6 bg-white border-b border-gray-200 h-96 overflow-scroll">
                    ...
                    <button role="submit"
                      ...
                      id="submitRequestBarangayCert">Submit</button>
                    </div>
                    <div class="flex flex-col  w-full">
                      <button class="p-3 bg-red-500 text-white hover:bg-red-600 mb-2 rounded-md"
                        id="cancelRequestBarangayCert">Cancel</button>
                    </div>

                    
                    <div class="flex flex-col  w-full">
                      <button class="p-3 bg-green-500 text-white hover:bg-green-600 rounded-md"
                        id="generateRequestBarangayCert">Generate Certificate</button>
                    </div>
      ...
      document.getElementById("barangay-cert-table").addEventListener("click", e => {
        e.preventDefault();
        if (e.target.id == "view-request-details") {
          document.getElementById("verify-barangay-cert-modal").classList.remove("hidden");
      ...
      document.getElementById("cancelRequestBarangayCert").addEventListener("click", e => {
        e.preventDefault();
        document.getElementById("verify-barangay-cert-modal").classList.add("hidden");
        console.log("cancel");
      });

      document.getElementById("submitRequestBarangayCert").addEventListener("click", e => {
        e.preventDefault();
        ...
        db.collection('barangays').doc(barangayDoc).collection('requests').doc(requestID).update({
          ...
          if (statusValue == "Completed" || statusValue == "Failed") {
            db.collection('barangays').doc(barangayDoc).collection('users').doc(authorID).update({
              barangay_certificate: false,
            }).then(function () {
              console.log("Indigency Certificate set to False:");
            })
          } else {
            db.collection('barangays').doc(barangayDoc).collection('users').doc(authorID).update({
              barangay_certificate: true,
            }).then(function () {
        ...
        document.getElementById('certificate-barangay').click();
      ...
      document.getElementById("generateRequestBarangayCert").addEventListener("click", e => {
        if (document.getElementById("pickupDateValue").value != "" && document.getElementById('requestStatus').value == "Pickup") {
          db.collection('barangays').doc(barangayDoc).collection('requests').doc(requestID)
            ...
            .then(doc => {
              ...
              localStorage.setItem("certificateType", "Barangay Certificate");
              ...
              document.getElementById("submitRequestBarangayCert").click();
            })
