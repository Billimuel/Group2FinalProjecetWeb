document.getElementById("certificate-clearance").addEventListener("click", e => {
  console.log("qqqqq");
  ...
  <div
    ...
    <table class="max-w-full min-w-full">
      ...
      <tbody class="bg-white"  id="barangay-clearance-table">
  `;

  var barangayDoc = localStorage.getItem('barangay');
  db.collection('barangays').doc(barangayDoc).collection('requests').where("type", "==", "barangay_clearance").get()
    .then((querySnapshot) => {
      ...
      <div class="w-full max-w-3xl p-5 relative mx-auto my-auto rounded-xl shadow-lg  bg-white pb-4">
        ...
        <div class="">
          ...
          <i
            ...
            <h2 class="text-xl font-bold py-2 ">Barangay Clearance</h3>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                  <div class="px-6 bg-white border-b border-gray-200 h-96 overflow-scroll">
                    ...
                    <button role="submit"
                      ...
                      id="submitRequestBarangayClearance">Submit</button>
                    </div>
                    <div class="flex flex-col  w-full">
                      <button class="p-3 bg-red-500 text-white hover:bg-red-600 mb-2 rounded-md"
                        id="cancelRequestBarangayClearance">Cancel</button>
                    </div>

                    <div class="flex flex-col  w-full">
                      <button class="p-3 bg-green-500 text-white hover:bg-green-600 rounded-md"
                        id="generateRequestBarangayClearance">Generate Certificate</button>
                    </div>
                  </div>
                </div>
              </div>
      ...
      document.getElementById("barangay-clearance-table").addEventListener("click", e => {
        e.preventDefault();
        if (e.target.id == "view-request-details") {
          document.getElementById("verify-clearance-modal").classList.remove("hidden");
      ...
      document.getElementById("cancelRequestBarangayClearance").addEventListener("click", e => {
        e.preventDefault();
        document.getElementById("verify-clearance-modal").classList.add("hidden");
        console.log("cancel");
      });

      document.getElementById("submitRequestBarangayClearance").addEventListener("click", e => {
        e.preventDefault();
        ...
        document.getElementById('certificate-clearance').click();

        document.getElementById("verify-clearance-modal").classList.add("hidden");
        console.log("submit");
      });

      document.getElementById("generateRequestBarangayClearance").addEventListener("click", e => {
        if (document.getElementById("pickupDateValue").value != "" && document.getElementById('requestStatus').value == "Pickup") {
          db.collection('barangays').doc(barangayDoc).collection('requests').doc(requestID)
            ...
            .then(doc => {
              ...
              localStorage.setItem("certificateType", "Barangay Clearance");
              ...
              document.getElementById("submitRequestBarangayClearance").click();
            })
